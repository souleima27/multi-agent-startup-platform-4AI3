...
class NotionAdapter(BaseAdapter):
    def execute(self, operation: str, payload: Dict[str, object], credentials: Dict[str, str]) -> AdapterResponse:
        url = payload.get("url")
        if not url:
            return AdapterResponse(success=False, retryable=False, error="Missing Notion url in payload")

        method = str(payload.get("method", "POST")).upper()
        headers = {
            "Authorization": f"Bearer {credentials.get('api_key', '')}",
            "Notion-Version": str(payload.get("notion_version", "2022-06-28")),
        ...
        try:
            with urllib.request.urlopen(request, timeout=30) as response:
                ...
                return AdapterResponse(
                    ...
                    external_ref_id=str(data.get("id") or ""),
                    retryable=False,
